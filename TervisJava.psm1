function Install-TervisJavaDeploymentRuleSet {
    ...
    begin {
        $JavaKeystoreCredential = (Get-PasswordstatePassword -ID 4282).Password
        ...
        if ($JavaHomeDirectory) {
            Invoke-Command -ComputerName $ComputerName -ScriptBlock {
                ...
                if (Test-Path -Path $using:JavaHomeDirectory\bin\keytool.exe) {
                    . "$KeytoolPath" -importcert -file $CertPath -alias tervisselfsigned -keystore $KeyStorePath -storepass $Using:JavaKeystoreCredential -noprompt
                } else {
